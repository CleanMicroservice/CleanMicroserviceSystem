...
{
    public class AphroditeJsonWebTokenParser
    {
        private readonly ILogger<AphroditeJsonWebTokenParser> logger;

        public AphroditeJsonWebTokenParser(ILogger<AphroditeJsonWebTokenParser> logger)
        {
            this.logger = logger;
        }
        ...
        public IEnumerable<Claim> ParseJWTToken(string token)
        {
            ...
            return parts?.Length != 3 ?
                ...
                ParseJWTPayload(parts[1]);
        }

        public IEnumerable<Claim> ParseJWTPayload(string payload)
        {
            logger.LogDebug($"Parse JWT Payload: {payload}");
